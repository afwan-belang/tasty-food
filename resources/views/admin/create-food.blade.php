...
<section class="min-h-screen py-20 px-6 bg-[#f4f4f4] flex items-center justify-center select-none anim-hero-entry">
    <div class="w-full max-w-xl bg-white rounded-[24px] p-8 sm:p-10 shadow-xl border border-gray-100">
        ...
        <div class="mb-8">
            ...
            <h2 class="text-2xl font-black text-gray-950 uppercase tracking-wide">TAMBAH DATA KULINER</h2>
        </div>
        ...
        <form action="{{ route('admin.food.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" name="title" value="{{ old('title') }}" required placeholder="Masukkan nama kuliner..." 
            ...
            <div>
                ...
                <select name="category" required class="w-full h-14 px-5 text-sm tasty-admin-input cursor-pointer @error('category') border-red-500 bg-red-50/50 focus:border-red-600 @enderror">
                    <option value="" disabled {{ old('category') == '' ? 'selected' : '' }}>Pilih Penempatan...</option>
                    <option value="card" {{ old('category') == 'card' ? 'selected' : '' }}>Food Card (Halaman Utama / Home)</option>
                    <option value="news" {{ old('category') == 'news' ? 'selected' : '' }}>Seksi Berita Kami (Home & Berita)</option>
                    <option value="gallery" {{ old('category') == 'gallery' ? 'selected' : '' }}>Asset Foto Galeri (Home & Galeri)</option>
                </select>
                @error('category')
                    <p class="text-red-600 text-[11px] mt-1.5 font-bold tracking-wide">⚠️ {{ $message }}</p>
                @enderror
            ...
            <div>
                <label class="block text-xs font-black tracking-wider text-gray-950 uppercase mb-2">Upload Foto Makanan</label>
                ...
                @error('image')
                    <p class="text-red-600 text-[11px] mt-1.5 font-bold tracking-wide">⚠️ {{ $message }}</p>
                @else
                    <p class="text-[11px] text-gray-400 mt-1">Format file disarankan: JPG, PNG, WEBP, atau AVIF (Maks. 2MB)</p>
                @enderror
            </div>

            <div>
                <label class="block text-xs font-black tracking-wider text-gray-950 uppercase mb-2">Informasi / Deskripsi Lengkap</label>
                <textarea name="desc" required placeholder="Tuliskan informasi menu kuliner secara mendalam..." 
                          class="w-full h-40 px-5 py-4 text-sm tasty-admin-input resize-none @error('desc') border-red-500 bg-red-50/50 focus:border-red-600 @enderror">{{ old('desc') }}</textarea>
                @error('desc')
                    <p class="text-red-600 text-[11px] mt-1.5 font-bold tracking-wide">⚠️ {{ $message }}</p>
                @enderror
            </div>
