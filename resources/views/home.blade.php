...
@section('content')
<style>
    .tasty-home-hero { background-color: #f4f4f4; min-height: 700px; }
    @media (min-width: 768px) { .tasty-home-hero { height: 700px; } }
    .tasty-hero-line { width: 60px; height: 3px; background-color: #1c1c1c; }
    .tasty-hero-plate-exact { position: absolute; right: -12%; top: 0; height: 100%; width: auto; object-fit: contain; z-index: 1; pointer-events: none; user-select: none; }
    @media (max-width: 1024px) { .tasty-hero-plate-exact { right: -25%; } }
    @media (max-width: 640px) { .tasty-hero-plate-exact { right: -45%; opacity: 0.12; } }

    @keyframes tastyFadeUp { from { opacity: 0; transform: translateY(30px); } to { opacity: 1; transform: translateY(0); } }
    @keyframes tastyPlateIn { from { opacity: 0; transform: translateX(60px); } to { opacity: 1; transform: translateX(0); } }
    .anim-hero-entry { opacity: 0; animation: tastyFadeUp 1200ms cubic-bezier(0.16, 1, 0.3, 1) forwards; }
    ...
    .anim-hero-plate { opacity: 0; animation: tastyPlateIn 1500ms cubic-bezier(0.16, 1, 0.3, 1) 300ms forwards; }
</style>
...
<section class="relative w-full tasty-home-hero flex flex-col overflow-hidden z-10">
    ...
    <div class="relative z-10 w-full max-w-7xl mx-auto px-6 md:px-12 lg:px-24 flex-grow flex flex-col items-start justify-center pt-8 pb-20 md:pb-16">
        ...
        <p class="text-gray-600 text-sm lg:text-base leading-relaxed max-w-xl mb-10 font-normal anim-hero-entry anim-delay-3">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Phasellus ornare, augue eu rutrum commodo, dui diam convallis arcu, eget consectetur ex sem eget lacus. Nullam vitae dignissim neque, vel luctus ex.</p>
        <div class="anim-hero-entry anim-delay-4"><x-ui.button href="{{ route('tentang') }}" class="rounded-none bg-black text-white hover:bg-gray-900 px-10 py-4 font-bold tracking-widest transition duration-300">TENTANG KAMI</x-ui.button></div>
    </div>
    <img src="{{ asset('asset/img-4-2000x2000.avif') }}" class="tasty-hero-plate-exact anim-hero-plate" alt="Main Plate Layout">
</section>
...
<section class="py-20 text-center bg-white anim-hero-entry anim-delay-5">
    ...
    <p class="text-gray-600 max-w-3xl mx-auto px-6 leading-relaxed text-sm lg:text-base">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Phasellus ornare, augue eu rutrum commodo, dui diam convallis arcu, eget consectetur ex sem eget lacus.</p>
    <div class="w-16 h-[3px] bg-gray-900 mx-auto mt-8"></div>
</section>

<div class="bg-white py-16 px-0">
    <section class="relative w-full aspect-none md:aspect-[12/4.9] overflow-visible flex items-center justify-center py-24 lg:py-0">
        <div class="absolute inset-0 z-0 pointer-events-none select-none reveal-on-scroll">
            <img src="{{ asset('asset/Group 70.avif') }}" class="w-full h-full object-cover object-center" alt="Background Master Banner">
        </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 items-center">
                @foreach ($foods as $food)
                    ...
                    <div data-id="{{ $food->id }}" onclick="openFoodModal(this.dataset.id)" class="reveal-on-scroll cursor-pointer {{ $delayClass }}">
                        <x-content.food-card :image="$food->image" :title="$food->title" :desc="Str::limit($food->desc, 90)" loading="lazy" />
                    </div>
                @endforeach
            </div>
        </div>
...
<section class="py-24 px-6 lg:px-24 bg-gray-50">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 max-w-7xl mx-auto">
        ...
        <div onclick="window.location.href='{{ route('berita.detail', $featuredNews->id) }}'" class="bg-white rounded-2xl shadow-lg overflow-hidden flex flex-col group cursor-pointer border border-gray-100 reveal-on-scroll">
            <div class="w-full h-[240px] sm:h-[320px] bg-gray-100 overflow-hidden"><img src="{{ asset($featuredNews->image) }}" loading="lazy" alt="{{ $featuredNews->title }}" class="w-full h-full object-cover group-hover:scale-105 transition duration-500"></div>
            <div class="p-6 sm:p-8 flex-grow flex flex-col justify-between">
                ...
                <div class="flex justify-between items-center pt-4 border-t border-gray-100"><x-ui.button variant="yellow">Baca selengkapnya</x-ui.button><span class="text-gray-300 font-bold tracking-widest text-lg">•••</span></div>
            </div>
        </div>
        @endif
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
            @foreach ($otherNews as $item)
                ...
                <div onclick="window.location.href='{{ route('berita.detail', $item->id) }}'" class="bg-white rounded-2xl shadow-md overflow-hidden flex flex-col group cursor-pointer border border-gray-100 reveal-on-scroll {{ $delayClass }}">
                    <div class="w-full h-44 bg-gray-100 overflow-hidden"><img src="{{ asset($item->image) }}" loading="lazy" alt="{{ $item->title }}" class="w-full h-full object-cover group-hover:scale-105 transition duration-500"></div>
                    <div class="p-6 flex-grow flex flex-col justify-between">
                        ...
                        <div class="flex justify-between items-center mt-2"><x-ui.button variant="yellow">Baca selengkapnya</x-ui.button><span class="text-gray-300 font-bold tracking-widest text-lg">•••</span></div>
                    </div>
                </div>
            @endforeach
        </div>
...
<section class="py-24 px-6 lg:px-24 bg-white">
    ...
    <div class="text-center reveal-on-scroll delay-100"><x-ui.button href="{{ route('galeri') }}">LIHAT LEBIH BANYAK</x-ui.button></div>
</section>
