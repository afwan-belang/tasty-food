...
<section class="relative w-full tasty-home-hero flex flex-col overflow-hidden z-10">    
    ...
    <div class="relative z-10 w-full max-w-7xl mx-auto px-6 md:px-12 lg:px-24 flex-grow flex flex-col items-start justify-center pt-8 pb-20 md:pb-16">
        ...
        <p class="text-gray-600 text-sm lg:text-base leading-relaxed max-w-xl mb-10 font-normal anim-hero-entry anim-delay-3">
            Tasty Food menghadirkan hidangan lezat dengan bahan berkualitas dan resep pilihan yang memadukan cita rasa lokal dan internasional. Kami percaya bahwa setiap makanan harus memberikan pengalaman yang tak terlupakan bagi setiap pelanggan.
        </p>
...
<section class="py-20 text-center bg-white anim-hero-entry anim-delay-5">
    ...
    <p class="text-gray-600 max-w-3xl mx-auto px-6 leading-relaxed text-sm lg:text-base">
        Tasty Food adalah restoran yang menghadirkan pengalaman kuliner istimewa dengan perpaduan rasa autentik dan sentuhan modern. Berdiri dengan komitmen untuk menyajikan makanan berkualitas tinggi, kami menggunakan bahan-bahan segar yang dipilih secara teliti untuk memastikan setiap hidangan memiliki rasa terbaik.
    </p>
                ...
                @foreach ($foods as $food)
                    ...
                    <div data-id="{{ $food->id }}" onclick="openFoodModal(this.dataset.id)" class="reveal-on-scroll cursor-pointer {{ $delayClass }}">
                        <x-content.food-card 
                            :image="$food->image" 
                            :title="$food->title" 
                            :desc="Str::limit($food->desc, 90)" loading="lazy"
                        />
                    </div>
                @endforeach
            </div>
...
<section class="py-24 px-6 lg:px-24 bg-gray-50">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 max-w-7xl mx-auto">
        ...
        @if($featuredNews)
        <div onclick="openFoodModal({{ $featuredNews->id }})" class="bg-white rounded-2xl shadow-lg overflow-hidden flex flex-col group cursor-pointer border border-gray-100 reveal-on-scroll">
...
<section class="py-24 px-6 lg:px-24 bg-white">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 max-w-7xl mx-auto mb-16">
        ...
        @foreach ($gallery as $item)
            ...
            <div onclick="openFoodModal({{ $item->id }})" class="aspect-square bg-gray-200 rounded-2xl overflow-hidden shadow-md relative group cursor-pointer reveal-on-scroll {{ $delayClass }}">
                <img src="{{ asset($item->image) }}" 
                     loading="lazy"
